...
string Scanner::GetSource(int start, int end) {
  string ans;
  ans.append(std::to_string(start));
  ans.append("-");
  ans.append(std::to_string(end));
  ans.append(" `");
  ans.append(source_.substr(start, end - start));
  ans.append("`");
  return ans;
}
...
string Scanner::strip_source(const string& str) {
  int lo = 0;
  int n = str.size();
  while (lo < n && (str[lo] == ' ' || str[lo] == '\n' || str[lo] == '\t')) {
    lo++;
  }
  int hi = n - 1;
  while (hi >= 0 && (str[hi] == ' ' || str[hi] == '\n' || str[hi] == '\t')) {
    hi--;
  }
  CHECK(lo <= hi);
